...
> Обновлять при любых изменениях вёрстки сайтов. Последняя проверка: **2026-07-07** (code review).
...
**Адаптер:** `packages/adapters/magistrate.ts`

### URL

- Домен: `*.msudrf.ru`
- Карточка: `modules.php?name=sud_delo&op=cs&case_id=...&delo_id=1540005`
...
| Вкладки | `ul#tabs.bookmarks.lawcase > li` |
| Контент вкладок | `div.tab-content` |

### Вкладки

| Индекс `.tab-content` | Содержимое |
|---|---|
| `0` | ДЕЛО |
| `1` | ДВИЖЕНИЕ ДЕЛА |
| `2` | СТОРОНЫ ПО ДЕЛУ (ТРЕТЬИ ЛИЦА) |

### Таблицы

- Класс таблиц: `table.tablcont`
- Вкладка `0`: пары `ключ → значение` в двух колонках
- Вкладка `1`: события в колонках `Наименование события / Дата события / Результат события / Судья`
- Вкладка `2`: таблица **транспонирована**:
  - строка 1: роли
  - строка 2: имена / организации
  - строка 3: требования

### Подтверждённый живой пример

URL:
`https://35.perm.msudrf.ru/modules.php?name=sud_delo&op=cs&case_id=2414120&delo_id=1540005`

Извлечённые поля:
- Номер дела: `2-936/2026`
- Категория: `Споры, связанные с жилищными отношениями ...`
- Судья: `Смирнова Мария Николаевна`

---

## Главная страница мирового суда (`*.msudrf.ru`)

> Используется модулем `enrich:courts` для автозаполнения `courts.json`.
> Структура **отличается** от district/appeal/cassation — нет `#show` блока.

| Поле | Селектор | Примечание |
|---|---|---|
| Название суда | `span#court_name` | Fallback: `footer p` |
| Адрес | `h2:contains("Адрес") + p` | Под заголовком "Контактная информация" |
| Email | `a[href^="mailto:"]` | Так же, как у district (первый unique) |
| Телефоны | Текст `Телефон:` в `.content` | Несколько номеров у разных сотрудников |

### Подтверждённые примеры

**`6.perm.msudrf.ru` — Судебный участок № 6 Дзержинского судебного района г. Перми**
- Название: `Судебный участок № 6 Дзержинского судебного района г. Перми Пермского края`
- Адрес: `614067, г. Пермь, ул. Хабаровская, д. 64`
- Телефоны: `201-17-34`, `201-17-35`, `201-05-72`
- Email: `6dzerzhinsky@ms.permkrai.ru`

**`35.perm.msudrf.ru` — Судебный участок № 1 Свердловского судебного района г. Перми**
- Название: `Судебный участок № 1 Свердловского судебного района г. Перми Пермского края`
- Адрес: `614007, г. Пермь, ул. Рабоче-Крестьянская, д. 2`
- Телефоны: `7 (342) 216-93-19`, `210-77-81`
- Email: `1sverdlovsky@ms.permkrai.ru`
